...
- Follow-up seam fix: the first bridge pass detected 6/9-style hairline contacts but appended tiny bridge polygons separately, which still allowed visibility-polygon ray casting to treat adjacent footprints independently. Hairline connectors are now unioned into the effective obscuring geometry after base/touch ignore rules are applied, so separate footprints remain separate for terrain-touch semantics while sharing no microscopic LOS slit. Browser verification used `http://127.0.0.1:8044/heatmap?packet_id=official-event-companion-page-9&zone_id=attacker&source=edge&offset_inches=12`: the page rendered one heatmap image, 30 safe-zone outlines, and the 12-inch offset state.
- Follow-up edge-offset fix: deployment-edge heatmap offsets now use the buffered deployment-zone frontier instead of shifting each exposed segment independently. This measures the selected distance from corners too, so L-shaped deployment zones produce rounded quarter-circle offset transitions rather than stepped copies of the original zone edge. Regression tests cover the L-corner sample frontier and blanked heatmap exclusion area. Browser verification used `http://127.0.0.1:8046/heatmap?packet_id=official-event-companion-page-9&zone_id=attacker&source=edge&offset_inches=12`: the page rendered one heatmap raster and 23 `safe-zone-outline` elements. Decoding the embedded PNG confirmed a point 11.314 inches diagonally from the page-9 attacker L-corner was transparent (`alpha=0`), while a point 12.728 inches from the same corner remained heatmap-colored (`alpha=184`).

### Official Layout Organization Slice

- Added official layout metadata for Event Companion pages 9-53. The table covers 15 matchup series, each with Layout A/B/C, and records first-player force disposition, first-player primary mission, second-player force disposition, second-player primary mission, source page, and layout variant.
- Packet generation now preserves that metadata in `MapPacket.layout_metadata`, names official packets as `<first disposition> vs <second disposition> - Layout <variant>`, and includes the mission matchup in the source string.
- The web selectors now group official packets by force-disposition matchup and label each option by layout variant plus mission matchup. The repository sorts official packets by source page so page 9 remains the default after full ingestion.
- Full official ingestion regenerated 45 local packets from Event Companion pages 9-53 under `data/processed/map-packets/`. `data/processed/` remains ignored generated state; the committed source now contains the deterministic metadata and ingestion/UI code needed to reproduce those packets.
- Verification after the slice:
...
- In-app browser automation could not be repeated in this continuation because the Browser plugin runtime did not expose `globalThis.browser` and the expected `browser-client.mjs` file was absent from the local Codex plugin cache. The rendered-page checks exercise the same server-rendered HTML/SVG/PNG paths but are not a live browser-tab inspection.
