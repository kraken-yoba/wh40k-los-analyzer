...
| Dense official blockers and categories | Event Companion page 9 plus Terrain Area Footprints cache | `/api/layouts/event-companion-page-9`, `/footprint-matches`, `/los` | `backend/tests/domain/test_event_companion_extraction.py`, `backend/tests/domain/test_terrain_footprint_templates.py`, `backend/tests/test_gui.py` | Browser loaded page 9, showed 16 provisional match rows and `terrain_footprint_blocker_review_required`; API payload contained 386 provisional blockers, all restricted to Dense feature ids `terrain-01, terrain-03, terrain-07, terrain-08, terrain-12, terrain-14`; map-click LOS stayed blocked by unresolved review warnings, console errors were empty, and 390px mobile viewport had no horizontal overflow | Pass |
