...
from warhammer_companion.application.roster_snapshots import (
    ROSTER_IMPORT_TOOLKIT_SCHEMA_VERSION,
...
def build_roster_import_result_from_bytes(
    *,
    ...
    army = (
        canonical_army_with_source_refs(army=parsed.army, source_ref_ids=source_ref_ids)
        if parsed.army is not None
        else None
    )
    roster_index = (
        build_canonical_roster_index(army=army, source_ref_ids=source_ref_ids)
        if army is not None
        else None
    )
