...
def extract_layout_from_pdf(
    ...
    *,
    ...
    try:
        ...
        official_features = _extract_official_feature_labels(
            page,
            transform,
            terrain_areas,
...
def _extract_official_feature_labels(
    page: Any,
    transform: BoardTransform,
    terrain_areas: Sequence[LayoutElement],
    *,
    raster_features: Sequence[LayoutElement] = (),
    source_page: int,
) -> list[LayoutElement]:
    elements: list[LayoutElement] = []
    ...
    for block in text_payload["blocks"]:
        if block.get("type") != 0:
            continue
        code = _official_feature_code_from_text(
            " ".join(
                str(span["text"])
...
def _official_feature_code_from_text(text: str) -> OfficialFeatureCode | None:
...
def _feature_anchor_center(feature: LayoutElement) -> Point:
