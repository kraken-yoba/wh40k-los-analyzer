...
def rasterize_svg(svg: str) -> bytes:
    ...
    for element in list(root):
        ...
        if tag == "rect":
            _draw_rect(canvas, element)
        elif tag == "image":
            _draw_image(canvas, element)
        elif tag == "polygon":
            _draw_polygon(canvas, element)
        elif tag == "polyline":
            _draw_polyline(canvas, element)
        elif tag == "line":
            _draw_line(canvas, element)
        elif tag == "circle":
            _draw_circle(canvas, element)
        elif tag == "text":
...
def _draw_rect(canvas: Image.Image, element: ElementTree.Element) -> None:
    x = _number(element, "x")
    y = _number(element, "y")
    ...
    stroke_width = int(round(_number(element, "stroke-width", 1)))
...
def _draw_image(canvas: Image.Image, element: ElementTree.Element) -> None:
    ...
    width = int(round(_number(element, "width", image.width)))
    height = int(round(_number(element, "height", image.height)))
    if image.size != (width, height):
        image = image.resize((width, height), Image.Resampling.BILINEAR)
    canvas.alpha_composite(
        image, (int(round(_number(element, "x"))), int(round(_number(element, "y"))))
    )


def _draw_polygon(canvas: Image.Image, element: ElementTree.Element) -> None:
...
def _draw_polyline(canvas: Image.Image, element: ElementTree.Element) -> None:
    points = _points(element)
    stroke = _color(element, "stroke", "stroke-opacity")
...
def _draw_line(canvas: Image.Image, element: ElementTree.Element) -> None:
    stroke = _color(element, "stroke", "stroke-opacity")
    if not stroke:
        return
    stroke_width = int(round(_number(element, "stroke-width", 1)))
...
def _draw_circle(canvas: Image.Image, element: ElementTree.Element) -> None:
...
def _draw_text(canvas: Image.Image, element: ElementTree.Element) -> None:
    text = element.text or ""
    if not text:
        return
    fill = _color(element, "fill", "fill-opacity") or (28, 37, 32, 255)
    font_size = int(round(_number(element, "font-size", 13)))
    font = _font(font_size)
    x = _number(element, "x")
    y = _number(element, "y")
    css_class = element.attrib.get("class", "")
    stroke_width = 2 if "label" in css_class else 0
