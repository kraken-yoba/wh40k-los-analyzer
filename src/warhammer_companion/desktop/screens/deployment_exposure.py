...
from warhammer_companion.application.view_models import DeploymentExposureState
...
class DeploymentExposureScreen(QWidget):
    def __init__(self, service: WarhammerCompanionService) -> None:
        ...
        title = QLabel("Deployment Exposure")
    ...
    def refresh(self) -> None:
        state = self.service.deployment_exposure_state(
    ...
    def _apply_inputs(self, state: DeploymentExposureState) -> None:
    ...
    def _populate_deployment_zones(self, state: DeploymentExposureState) -> None:
    ...
    def _populate_modes(self, state: DeploymentExposureState) -> None:
    ...
    def _set_status(self, state: DeploymentExposureState) -> None:
        placement = (
            "Not exposed under selected assumptions"
            if state.not_exposed_under_assumptions
            else "Exposed or blocked under selected assumptions"
        )
        probability = state.threat_probability_at_center * 100.0
        details = "; ".join(state.placement_reason_details)
        warnings = " ".join(state.warning_details)
        self.status_label.setText(
            f"{placement}. Threat probability at center: {probability:.1f}%. {details} {warnings}"
        )
